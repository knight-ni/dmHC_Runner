...
func (osinfo *OSInfo) in(target string, strarray []string) bool {
...
func (osinfo *OSInfo) getKeys(m map[string]map[string]interface{}) []string {
...
func (osinfo *OSInfo) GetNCInfo() ([]string, []map[string]interface{}) {
	...
	for i = 0; i < len(v); i++ {
		if arrays.Contains(v[i].Flags, "up") > -1 {
			x, _ := ethHandle.CmdGetMapped(v[i].Name)
			if x != nil {
...
//
//	var i int
//	collst = []string{"Pid", "Parent", "Ctime", "Name", "Cmd", "Env", "Cwd", "CpuPct", "Nice", "Username", "Groups", "NumThreads", "NumFDs", "Terminal"}
//
//	for i = 0; i < len(v); i++ {
//		skipflag := true
//		if v[i].Pid == 0 {
...
//		Cwd, err := v[i].Cwd()
//		if err != nil {
//			mylog.Warnf("process cwd info collect failed! error:%s\n", err)
//			return nil, nil
//		}
...
//		if err != nil {
//			mylog.Warnf("process username info collect failed! error:%s\n", err)
//			return nil, nil
//		}
//		Groups, err := v[i].Groups()
//		if err != nil {
//			mylog.Warnf("process groups info collect failed! error:%s\n", err)
//			return nil, nil
//		}
//		NumThreads, err := v[i].NumThreads()
//		if err != nil {
//			mylog.Warnf("process numthreads info collect failed! error:%s\n", err)
//			return nil, nil
//		}
//		NumFDs, err := v[i].NumFDs()
//		if err != nil {
//			mylog.Warnf("process numfds info collect failed! error:%s\n", err)
//			return nil, nil
//		}
//		Terminal, err := v[i].Terminal()
//		if err != nil {
//			mylog.Warnf("process terminal info collect failed! error:%s\n", err)
//			return nil, nil
//		}
//
//		if skipflag {
